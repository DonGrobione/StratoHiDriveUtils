...
There is no build step. `Tests/DonGrobione.StratoHiDriveUtils.Tests.ps1` is the Pester 5 test suite; it runs in Windows PowerShell 5.1 and covers the manifest and `FileList` against the release workflow's shipped files, parsing, PSScriptAnalyzer, comment-based help, every exported function, and the installer, using `TestDrive` sandboxes and mocked GitHub calls. Extend it whenever behavior changes, and never let tests touch real installations, the real `PSModulePath`, or the network. Before finishing work, run the suite and these checks with Windows PowerShell 5.1 (not pwsh). The manifest must pass `Test-ModuleManifest`, every file must parse, and PSScriptAnalyzer must report no findings of any severity, as required for PowerShell Gallery publishing:
